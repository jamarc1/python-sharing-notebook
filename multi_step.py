# -*- coding: utf-8 -*-
"""
Created on Sun Aug  4 11:23:43 2019
...
def forecast_lstm(model, X, n_lag):
   ...
   X = X.reshape(1, n_lag, len(X)/n_lag)
   # make forecast
...
def inverse_transform(series, forecasts, scaler, n_test):
   ...
   for i in range(len(forecasts)):
      ...
      forecast = forecast.reshape(5, 3)
      # invert scaling
      #X : array-like, shape [n_samples, n_features]
      inv_scale = scaler.inverse_transform(forecast)
      #inv_scale = inv_scale[0, :]
      # invert differencing
      index = len(series) - n_test + i - 1  # Finds index of last training object
      last_ob = series.values[index] # gets values of last object
...
raw_data = raw_data[['order_qty','demand_frcst','ohi']]
# configure
n_lag = 20
n_seq = 5        # Number of time steps to take
...
actual = [row[-n_seq*n_features:] for row in test]
